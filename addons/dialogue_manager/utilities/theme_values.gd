...
static func get_values_from_editor() -> DMThemeValues:
	...
	return DMThemeValues.new({
		...
		# Headless-editor compat: the setting may be unset (Nil) outside the GUI editor.
		font_size = editor_settings.get_setting("interface/editor/code_font_size") if editor_settings.get_setting("interface/editor/code_font_size") != null else 16
	})
